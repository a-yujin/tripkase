	...
	<tr align="center" height="20">
		<td colspan="6">
			<c:if test ="${currentPage != 1}">
				<a href="/restaurant/${urlVal }.tripkase?page=${currentPage - 1 }&areaValue=${areaValue}&searchValue=${searchValue}&typeValue=${typeValue}">[이전]</a>
			</c:if>
			<c:forEach var="p" begin="${startNavi }" end="${endNavi }">
				...
				<c:if test="${currentPage ne p }">
					<a href="/restaurant/${urlVal }.tripkase?page=${p }&areaValue=${areaValue}&searchValue=${searchValue}&typeValue=${typeValue}">${p }</a>
				</c:if>
			</c:forEach>
			<c:if test ="${maxPage > currentPage}">
			<a href="/restaurant/${urlVal }.tripkase?page=${currentPage + 1 }&areaValue=${areaValue}&searchValue=${searchValue}&typeValue=${typeValue}">[다음]</a>
			</c:if>
		</td>
	</tr>
</table>
