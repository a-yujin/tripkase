...
public class courseController {
	...
	public ModelAndView insertCourse(
		...
		if(!courseFileName.equals("")) {
			String root = request.getSession().getServletContext().getRealPath("resourses");
			...
			String boardFilepath = savePath + "\\"+ courseFileReName;
			course.setCourseFileName(courseFileName);
			course.setCourseFileReName(courseFileName); // 중복 파일 업로드를위해 파일 이름을 업로드 시간으로 설정
			course.setCourseFilePath(boardFilepath);
			} 
